...
RGBA Menu::Settings::optionRect{ 10, 10, 10, 170 };
...
RGBA Menu::Settings::scroller{ 250, 0, 0, 230 };
RGBAF Menu::Settings::integre{ 250, 255, 255, 255, 2 };
RGBA Menu::Settings::line{ 250, 0, 0, 255 };
...
void Menu::BANNER()
{
	...
	std::ostringstream m_FilePath; m_FilePath << std::getenv("appdata") << "\\MARSBAR\\BANNER\\" << file;

	int textureID;
	if (FileExists(m_FilePath.str().c_str()))
		file_register(&textureID, m_FilePath.str().c_str(), true, file, false);

	GRAPHICS::REQUEST_STREAMED_TEXTURE_DICT("BANNER", false);
}

void Menu::SUBTITLE()
{
	const char* file = "SUBTITLE.ytd";
	std::ostringstream m_FilePath; m_FilePath << std::getenv("appdata") << "\\MARSBAR\\SUBTITLE\\" << file;

	int textureID;
	if (FileExists(m_FilePath.str().c_str()))
		file_register(&textureID, m_FilePath.str().c_str(), true, file, false);

	GRAPHICS::REQUEST_STREAMED_TEXTURE_DICT("SUBTITLE", false);
}

void Menu::ENDBAR()
{
	const char* file = "ENDBAR.ytd";
	std::ostringstream m_FilePath; m_FilePath << std::getenv("appdata") << "\\MARSBAR\\ENDBAR\\" << file;

	int textureID;
	if (FileExists(m_FilePath.str().c_str()))
		file_register(&textureID, m_FilePath.str().c_str(), true, file, false);

	GRAPHICS::REQUEST_STREAMED_TEXTURE_DICT("ENDBAR", false);
}
...
{
	Drawing::Spriter("BANNER", "BANNER", Settings::menuX, 0.1175f, Menu::Settings::menuXscale, 0.085f, 0, 255, 255, 255, 255);
	...
	if (Menu::Settings::currentOption <= Menu::Settings::maxVisOptions && Menu::Settings::optionCount <= Menu::Settings::maxVisOptions) {
		if (*prev != cur)
		{
			if (prevf < curf)
			{
				...
				GRAPHICS::DRAW_RECT(Menu::Settings::menuX, prevf, 0.22f, 0.035f, Menu::Settings::scroller.r, Menu::Settings::scroller.g, Menu::Settings::scroller.b, Menu::Settings::scroller.a);
				if (prevf >= curf)
				{
					prevf = curf;
					*prev = cur;
		...
		{
			*prev = cur;
			GRAPHICS::DRAW_RECT(Menu::Settings::menuX, cur * 0.035f + 0.1768f, 0.22f, 0.035f, Menu::Settings::scroller.r, Menu::Settings::scroller.g, Menu::Settings::scroller.b, Menu::Settings::scroller.a);
		}
	}
		...
		GRAPHICS::DRAW_RECT(Menu::Settings::menuX, ((Menu::Settings::optionCount - (Menu::Settings::currentOption - Menu::Settings::maxVisOptions)) * 0.035f + 0.1768f), 0.22f, 0.035f, Menu::Settings::scroller.r, Menu::Settings::scroller.g, Menu::Settings::scroller.b, Menu::Settings::scroller.a);
	}
};
...
bool Menu::Option(const char* option)
{
	...
	{
		Drawing::Text(option, Settings::optionText, { Settings::menuX - 0.095f, (Settings::optionCount) * 0.035f + 0.163f }, { 0.40f, 0.40f }, false);
		...
		if (onThis)
		{
			//Drawing::Rect(Settings::scroller, { Settings::scrollbarX, (Settings::optionCount) * 0.035f + 0.1768f }, { 0.008f, 0.025f });
			Drawing::Rect(Settings::scroller, { Settings::menuX, (Settings::optionCount) * 0.035f + 0.1768f }, { 0.20f, 0.035f });
		}
	}
	else if (Settings::optionCount > (Settings::currentOption - 13) && Settings::optionCount <= Settings::currentOption)
	{
		Drawing::Text(option, Settings::optionText, { Settings::menuX - 0.095f, (Settings::optionCount - (Settings::currentOption - 13)) * 0.035f + 0.163f }, { 0.40f, 0.40f }, false);
		...
		if (onThis)
		{
			Drawing::Rect(Settings::scroller, { Settings::menuX,  (Settings::optionCount - (Settings::currentOption - 13)) * 0.035f + 0.1768f }, { 0.20f, 0.035f });
			//Drawing::Rect(Settings::scroller, { Settings::scrollbarX,  (Settings::optionCount - (Settings::currentOption - 11)) * 0.035f + 0.1768f }, { 0.008f, 0.025f });
		}
	}
...
bool Menu::SmallTitle(const char* option)
{
	...
	if (Settings::showSmalltitle);
	{
		...
		Drawing::Text(Version, Settings::count, { Settings::menuX - 0.095f, (Settings::optionCount) * 0.035f + 0.164f }, { 0.38f, 0.38f }, false);
...
bool Menu::Toggle(const char* option, bool& b00l)
{
	...
	if (Settings::currentOption <= Settings::maxVisOptions && Settings::optionCount <= Settings::maxVisOptions)
		Drawing::Spriter("commonmenu", b00l ? "common_medal" : "common_medal", (Settings::menuX + 0.090f), (Settings::optionCount * 0.035f + 0.177f), 0.015f, 0.025f, 0, b00l ? 0 : 255, b00l ? 255 : 0, 0, 255);
		...
		Drawing::Spriter("commonmenu", b00l ? "common_medal" : "common_medal", (Settings::menuX + 0.090f), ((Settings::optionCount - (Settings::currentOption - 13)) * 0.035f + 0.177f), 0.015f, 0.025f, 0, b00l ? 0 : 255, b00l ? 255 : 0, 0, 255);
...
bool Menu::Int(const char* option, int& _int, int min, int max)
{
	...
	if (Settings::currentOption <= Settings::maxVisOptions && Settings::optionCount <= Settings::maxVisOptions)
		Drawing::Text(Tools::StringToChar("< " + std::to_string(_int) + " >"), Settings::integre, { Settings::menuX + 0.078f, Settings::optionCount * 0.035f + 0.165f }, { 0.32f, 0.32f }, true);
	else if (Settings::optionCount > Settings::currentOption - Settings::maxVisOptions && Settings::optionCount <= Settings::currentOption)
		Drawing::Text(Tools::StringToChar("< " + std::to_string(_int) + " >"), Settings::integre, { Settings::menuX + 0.078f, (Settings::optionCount - (Settings::currentOption - 13)) * 0.035f + 0.165f }, { 0.32f, 0.32f }, true);
...
void Menu::End()
{
	...
	if (opcount >= 13) 
	{
		Drawing::Text(Tools::StringToChar(std::to_string(currop) + " / " + std::to_string(opcount)), Settings::count, { Settings::menuX + 0.086f, 15 * 0.035f + 0.129f }, { 0.35f, 0.35f }, true);
		Drawing::Rect(Settings::optionRect2, { Settings::menuX, 15 * 0.035f + 0.1415f }, { 0.20f, 0.035f });
		Drawing::Rect(Settings::line, { Settings::menuX, 15 * 0.035f + 0.1235f }, { 0.20f, 0.00132f });
		Drawing::Spriter("commonmenu", "shop_arrows_upanddown", Settings::menuX, ((13 + 2) * 0.035f + 0.142f), 0.020f, 0.035f, 180, Settings::count.r, Settings::count.g, Settings::count.b, Settings::count.a);
		//Creating bottom scrollbar arrow box
		//Drawing::Rect({ 0,0,0,220 }, { ScrollbarBGX, ((13 + 1) * 0.035f + 0.1660f) }, { Menu::Settings::ScrollerWidth, 0.014f });//-------
		//Drawing::Spriter("commonmenutu", "arrowleft", ScrollbarBGX + 0.000021, ((13 + 1) * 0.035f + 0.1670f), 0.008f, 0.012f, -90, Settings::arrow2.r, Settings::arrow2.g, Settings::arrow2.b, Settings::arrow2.a);
	}
	else if (opcount > 0) 
	{
		Drawing::Text(Tools::StringToChar(std::to_string(currop) + " / " + std::to_string(opcount)), Settings::count, { Settings::menuX + 0.086f, (Settings::optionCount + 2) * 0.035f + 0.129f }, { 0.35f, 0.35f }, true);
		Drawing::Rect(Settings::optionRect2, { Settings::menuX, (Settings::optionCount + 2) * 0.035f + 0.1415f }, { 0.20f, 0.035f });
		Drawing::Rect(Settings::line, { Settings::menuX, (Settings::optionCount + 2) * 0.035f + 0.1235f }, { 0.20f, 0.00132f });
		...
		//Creating bottom scrollbar arrow box - else if
		//Drawing::Rect({ 0,0,0,220 }, { ScrollbarBGX, ((Settings::optionCount + 1) * 0.035f + 0.1660f) }, { Menu::Settings::ScrollerWidth, 0.014f });//-------
		//Drawing::Spriter("commonmenutu", "arrowleft", ScrollbarBGX + 0.000021, ((Settings::optionCount + 1) * 0.035f + 0.1670f), 0.008f, 0.012f, -90, Settings::arrow2.r, Settings::arrow2.g, Settings::arrow2.b, Settings::arrow2.a);
	}
}
